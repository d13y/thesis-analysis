...
surveyControlNO <- surveyControl %>% filter(OUTLIER==0)
surveyTreatmentNO <- surveyTreatment %>% filter(OUTLIER==0)

#Cohen's D
statutoryD <- cohens_d(surveyControlNO$CERTAINTY_STATUTORY,
                       surveyTreatmentNO$CERTAINTY_STATUTORY)
propertyD <- cohens_d(surveyControlNO$CERTAINTY_PROPERTY,
                      surveyTreatmentNO$CERTAINTY_PROPERTY)
personsD <- cohens_d(surveyControlNO$CERTAINTY_PERSONS,
                      surveyTreatmentNO$CERTAINTY_PERSONS)
...
crimegazer(olsStatutory, olsProperty, olsPersons,
           ...
           caption="Table EEE: OLS Regression Results, Outlier Flag")
```